...
function getPosts(req, res) {
    ...
    if (id) {
        ...
        Profile.findOne({ username: userObj.username }).exec(function(err, userProfile) {
            ...
            if (whichPage == Number.MAX_SAFE_INTEGER) {
                Post.find({}).where('author')
                    .in(userProfile.following).count(function(err, count) {
                        var limit = count%10 == 0? 10: count%10;
                        ...
                        Post
                            ...
                            .limit(limit)
                            .skip(skip) // the whichPage page, skip the first whichPage-1 records, one page 10 docs
                            .sort('-date')
                            .exec(function(err, posts) {
                                return res.send({ posts: postWithId(posts, false), whichPage: Math.floor(count/10)+1 });
                            })
                    })
                    return;
            } 
